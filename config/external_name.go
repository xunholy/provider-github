...
var ExternalNameConfigs = map[string]config.ExternalName{
	...
	"github_repository_autolink_reference": config.TemplatedStringAsIdentifier("autolink_reference", "{{ .parameters.repository }}:{{ .external_name }}"),
	"github_repository_deploy_key":         config.IdentifierFromProvider,
	"github_repository_file":               config.TemplatedStringAsIdentifier("file", "{{ .parameters.repository }}/{{ .parameters.file }}:{{ .parameters.branch }}"),
	...
	"github_repository_webhook":            config.TemplatedStringAsIdentifier("webhook", "{{ .parameters.repository }}/{{ .external_name }}"),
	"github_team":                          config.IdentifierFromProvider,
	"github_team_members":                  config.IdentifierFromProvider,
	"github_team_membership":               config.TemplatedStringAsIdentifier("team_membership", "{{ .parameters.team_id }}:{{ .parameters.username }}"),
	"github_team_repository":               config.TemplatedStringAsIdentifier("team_repository", "{{ .parameters.team_id }}:{{ .parameters.repository }}"),
